...
class Gst
{
    ...
    {
        // JORGE FLORES -> MODIFICAR PARA Q EL IMPUESTO SIGA ESTANDO EN $
        $query = $this->db->query("SELECT * FROM sma_exchange_rate where id =1");
        $row = $query->row();
        if (isset($row)){
            $tasa_dolar = $row->tasa;          
        }
        ...
        if (!empty($tax_summary)) {
            ...
            foreach ($tax_summary as $summary) {
                $html .= '<tr><td>' . $summary['name'] . '</td><td style="text-align:center;">' . $summary['code'] . '</td><td class="text-center">' . $this->sma->formatQuantity($summary['items']) . '</td><td style="text-align:right;">' . $this->sma->formatMoney($summary['amt']) . '</td><td class="text-right">' . $this->sma->formatMoney($summary['tax'])*$tasa_dolar . '</td></tr>';
            }
            $html .= '</tbody></tfoot>';
