<?php
//JORGE FLORE DEV 01-04-2025
//CAMBIOS : EN LA FUNCION -> getSalesReport ()
...
class Reports extends MY_Controller
{
    ...
    public function getSalesReport($pdf = null, $xls = null)
    {
        ...
        if ($pdf || $xls) {
            $this->db
             // JORGE FLORE DEV -> CAMBIAR QUANTITY POR unit_price
                ->select('date, reference_no, biller, customer, GROUP_CONCAT(CONCAT(' . $this->db->dbprefix('sale_items') . ".product_name, ' (', " . $this->db->dbprefix('sale_items') . ".quantity, ' x $', " . $this->db->dbprefix('sale_items') . ".unit_price,  ')') SEPARATOR '\n') as iname, grand_total, paid, payment_status", false)
                ->from('sales')
            ...
            // JORGE FLORE DEV -> CAMBIAR QUANTITY POR unit_price
            $si = "( SELECT sale_id, product_id, serial_no, GROUP_CONCAT(CONCAT({$this->db->dbprefix('sale_items')}.product_name, '__', {$this->db->dbprefix('sale_items')}.quantity,'__', {$this->db->dbprefix('sale_items')}.unit_price) SEPARATOR '___') as item_nane from {$this->db->dbprefix('sale_items')} ";
            if ($product || $serial) {
                $si .= ' WHERE ';
            }
            if ($product) {
