...
if($accion==1){
    if($datos==1){       
        $idBrigada = strtoupper($_POST['idBrigada']);
        $unidad = strtoupper($_POST['unidad']); 
        $f = new FuncionesData();
        $t = $f->foundValor("unidad","unidad",$unidad,"UnidadData");
        $id = $t->cuenta;
        if($id==1){
            echo '2';         
        }else{
            $objeto = new UnidadData(); 
            $objeto->idBrigada =$idBrigada;             
...
if($accion==3){
    if($datos==1){       
        ...
        $f = new FuncionesData();
        $t = $f->foundValorxValor("unidad","unidad",$unidad,"idBrigada",$idBrigada,"UnidadData");
        $token = $t->cuenta;
        if($token==1){ 
            echo '1';         
        }else{           
