...
function graficoReporteXA(meses,meses2,grafico){

  //console.log(meses);
  //console.log(meses2);
let titulo = meses2[0]+'-'+meses[0];
...
$('.btnCambiarCliente').on('click', function (e) {
      
  //let id = $('#idContratoData').val();
  let idCliente = $('#cmbCambiarCliente_idCliente').val();
  let id = $('#idVehiculo').val();
  if ((idCliente!= '0') ) { 
 // let codigoContrato = $('#codigoContrato3').val();
  //(idp);
   e.preventDefault();
   Swal.fire({
     title: '¿Deseas cambiar el cliente?',
     text: "Tenga en cuenta que está realizando el cambio del cliente del actual equipo, el mismo acarreara cambios internos de control y seguimiento.",
     icon: 'warning',
     showCancelButton: true,
     confirmButtonColor: '#3085d6',
     cancelButtonColor: '#d33',
     ...
     if (result.isConfirmed) {
        //  pagar();
        cambiarClienteItem(id,idCliente);
        $('.modal').modal('hide');
        //dt_basic.row($(this).parents('tr')).remove().draw();
    ...
    Swal.fire({
      icon: 'error',
      title: 'Oops...',
      text: 'Si desea cambiar de cliente, debe elegir el nuevo  de la lista despleglable!'
...
if ($('.dataEmpresa').length) {
  ...
  if ($('#buscarDataUbicacionesOrdenes').length) {    
    cargarDataUbicacionesOrdenes();
  }
...
if ($('#cmbAddOrden_idUbicacion').length) {
  ...
  if ($('#cmbCambiarCliente_idCliente').length) {
    cargarClientes('.cmbCambiarCliente_idCliente', 0);
  }
...
function cambiarClienteItem(id,idCliente) {
  var tipo = 1;
  var accion = 3;
  var datos = 2;
  $.ajax({
    url: '../admin/index.php?action=items',
    type: 'post',
    data: { tipo: tipo, accion: accion, datos: datos, id: id,idCliente:idCliente },
    success: function (response) {
      //alert(response);
      if (response == 1) {
        Swal.fire({
          icon: 'success',
          text: 'Se ha cambiado el cliente correctamente.'
        }),
        window.location = 'index.php?view=item&id='+id;
      } else {
        Swal.fire({
          icon: 'error',
          title: 'Oops...',
          text: 'Hubo un error al cambiar.'
        })
      }
...
function cargarDataUbicacionesOrdenes() {
 
  $.ajax({
    type: "GET",
    url: '../admin/index.php?action=ubicaciones&tipo=1&accion=2&datos=3&c=UbicacionData&a=1&t=ubicacion',
  }).done(function (data) {
    console.log(data);
    var i = 0;
    var j=0;
    ...
    let tope2 =0;
      ...
      let contenido2=``;
    for (var i = 0; i < tope; i++) {
        tope2 = data[i].items.length
         contenido=`<div class="col-md-6 col-12"  id="buscarOrdenesDetalles${data[i].id}">
                    <div class="card">
                    <div class="card-header border-bottom">
                    <h4 class="card-title">${data[i].ubicacion}</h4>
                     <h6 class="card-title">Equipos : ${tope2}</h6>
                    </div>
        ...
        if(tope2>=1){
         console.log(tope2)
        for (var j = 0; j < tope2; j++) {
          let tipo = data[i].items[j].tipo;
          let marca = data[i].items[j].marca;
          let modelo = data[i].items[j].modelo;
          let media = data[i].items[j].media;
          let idItem = data[i].items[j].idItem;
          let nombreCompleto = data[i].items[j].dato5;
           let fechaIngreso = data[i].items[j].fechaIngreso;
              contenido+=`<div class="d-flex mt-2">
                                            <div class="flex-shrink-0">
                                               <a  href="index.php?view=item&id=${idItem}"> <img src="${media}" alt="imagen Equipo" class="me-1" height="50" width="50" />  </a>
                                            </div>
                                            <div class="d-flex align-item-center justify-content-between flex-grow-1">
                                                <div class="me-1">
                                                    <p class="fw-bolder mb-0">${tipo} -${marca} - ${modelo}</p>
                                                    <span> ${nombreCompleto} </span>
                                                    <br>
                                                   <b> <span> ${fechaIngreso} </span></b>
                                                </div>
                                                <div class="mt-50 mt-sm-0">
                                                    <a  href="index.php?view=item&id=${idItem}" type="button" class="btn btn-icon btn-outline-secondary">
                                                        🔗
                                                    </a>
                                                </div>
                                            </div>
        ...
        $('#buscarOrdenesDetalles'+data[i].id).append(contenido);   
    ...
    $('#buscarDataUbicacionesOrdenes').append(contenido);   
...
function estadisticasMes_tablero(){
  ...
  $.ajax({
    type: "GET",
    url: '../admin/index.php?action=combos&c=ItemData&a=999&t=ordentrabajo', 
}).done(function(meses) { 
