...
if($accion ==1){
    if($datos==1){
        ...
        $cargo = strtoupper($_POST['cargo']); // valor para sacar el id del usuario
        $f = new FuncionesData();
        $t = $f->foundValor("cargo","cargo",$cargo,"CargoData");
        $id = $t->cuenta;
        if($id==1){
            echo "2";         
        }else{
            $objeto = new CargoData();           
            $objeto->cargo =$cargo;          
            $objeto->add();       
...
if($accion ==3){
    if($datos==1){
        ...
        $f = new FuncionesData();
        $t = $f->foundValorxValor("cargo","cargo",$cargo,"id",$id,"CargoData");
        $token = $t->cuenta;
        if($token==1){          
            echo "1";         
        }else{
