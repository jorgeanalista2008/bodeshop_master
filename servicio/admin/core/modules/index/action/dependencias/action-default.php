...
if($accion==1){
    if($datos==1){       
        $idUnidad = strtoupper($_POST['idUnidad']);
        $dependencia = strtoupper($_POST['dependencia']); 
        $f = new FuncionesData();
        $t = $f->foundValor("dependencia","dependencia",$dependencia,"DependenciaData");
        $id = $t->cuenta;
        if($id==1){
            echo '2';         
        }else{
            $objeto = new DependenciaData(); 
            $objeto->idUnidad =$idUnidad;             
...
if($accion==4){
    if($datos==1){
            ...
            $banco_objeto = new DependenciaData();           
            $banco_objeto->id =$id;                
            $banco_objeto->del();
           echo "1";
        }    
